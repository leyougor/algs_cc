...
unsigned int initialize_hash (unsigned int sizeVoc) {
	...
	fprintf(stderr,"\nHash Table initilized with: %ud elements",TAM_HASH);
...
unsigned int NearestPrime(unsigned int n)
{
    ...
    for (position = n; ; position++)
    {
        ...
        for (index = 2; index <= (int) sqrt((double) position) && position % index != 0; index++) ;
...
unsigned int hashFunction (const unsigned char *aWord, unsigned int len)
{
    ...
    last=((unsigned int) aWord) +len -1;

    h = SEED;
   
    for( ; ((unsigned int) aWord <=last ) ; )
    {
    	c=*(aWord++);
		//c=*aWord;
		h ^= ( (h << 5) + c + (h >> 2) );
