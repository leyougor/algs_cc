 ...
 */
#ifndef ALGS_UTILS_STEM_H_
#define ALGS_UTILS_STEM_H_
...
public:
    static string stem(const string &ss)
    {
        string str = ss, stoper = ",.:;!?\"'()[]@#$%^&*_+-=|<>\\/~{}";
        if (str.size() > 0)
        {
            while (str.size() > 0 && stoper.find(str.substr(0, 1)) != stoper.npos)
            {
                str = str.substr(1);
            }
            while (str.size() > 0 && stoper.find(str.substr(str.size() - 1)) != stoper.npos)
            {
        ...
        strcpy(ch, str.c_str());
        ch[st.stem(ch, 0, str.size() - 1) + 1] = 0;
